...
public class ResEdgeLet {

    /** The Cloudlet object. */
    public final EdgeLet edgelet;
     ...
     *
     * @param edgelet an edgelet object
     * @see gridsim.CloudSim#clock()
     * @pre cloudlet != null
     * @post $none
     */
    public ResEdgeLet(EdgeLet edgelet) {
     ...
     * @post $none
     */
    public ResEdgeLet(EdgeLet edgelet, long startTime, int duration, int reservID) {
        this.edgelet = edgelet;
    ...
    private void init() {
        ...
        pesNumber = edgelet.getNumberOfPes();
        ...
        edgelet.setSubmissionTime(arrivalTime);
        ...
        // hostList.
        cloudletFinishedSoFar = edgelet.getCloudletFinishedSoFar() * Consts.MILLION;
    }
    ...
    public long getCloudletLength() {
        return edgelet.getCloudletLength();
    }
    ...
    public long getCloudletTotalLength() {
        return edgelet.getCloudletTotalLength();
    }
    ...
    public boolean setCloudletStatus(int status) {
        ...
        int prevStatus = edgelet.getCloudletStatus();
        ...
        try {
            ...
            edgelet.setCloudletStatus(status);
            ...
            if (status == Cloudlet.INEXEC || (prevStatus == Cloudlet.PAUSED && status == Cloudlet.RESUMED)) {
                startExecTime = clock;
                edgelet.setExecStartTime(startExecTime);
            }
    ...
    public double getExecStartTime() {
        return edgelet.getExecStartTime();
    }
    ...
    public void setExecParam(double wallClockTime, double actualCPUTime) {
        edgelet.setExecParam(wallClockTime, actualCPUTime);
    }
    ...
    public long getRemainingCloudletLength() {
        long length = edgelet.getCloudletTotalLength() * Consts.MILLION - cloudletFinishedSoFar;
    ...
    public void finalizeCloudlet() {
        ...
        edgelet.setExecParam(wallClockTime, totalCompletionTime);

        long finished = 0;
        //if (cloudlet.getCloudletTotalLength() * Consts.MILLION < cloudletFinishedSoFar) {
        if (edgelet.getCloudletStatus()==Cloudlet.SUCCESS) {
            finished = edgelet.getCloudletLength();
        } else {
            finished = cloudletFinishedSoFar / Consts.MILLION;
        }

        edgelet.setCloudletFinishedSoFar(finished);
    }
    ...
    public int getCloudletStatus() {
        return edgelet.getCloudletStatus();
    }
